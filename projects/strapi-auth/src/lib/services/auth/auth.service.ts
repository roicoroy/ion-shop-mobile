...
export class AuthService {
   ...
   */
  public callbackProviderLogin(params: string, provider: StrapiAuthProviders): any {
    try {
      // const res: IResAuthLogin | void = (await lastValueFrom(
      //   this.authHttpClient.get(this.apiUrl + '/auth/' + provider + '/callback' + params)
      // )) as IResAuthLogin | void;
      return this.authHttpClient.get(this.apiUrl + '/auth/' + provider + '/callback' + params)
        .pipe(
          tap((res: any) => {
            console.log(res);
            if (res) {
